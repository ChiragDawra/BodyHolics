...
### D-015 — A scheduled broadcast whose audience resolves to nobody is CANCELLED

`docs/07` §6 defines the empty-audience outcome for the API path only: the call
fails with `BROADCAST_EMPTY_AUDIENCE` and nothing is published. The cron path had
no defined outcome, and leaving such a broadcast `SCHEDULED` would make
`publish_due_broadcasts()` retry it every minute forever, filling `audit_logs`
with the same failure.

`publish_due_broadcasts()` therefore moves it to the terminal `CANCELLED` state
and writes one `BROADCAST_CANCELLED_EMPTY_AUDIENCE` audit row. `CANCELLED` is a
legal transition from `SCHEDULED` in `docs/09` §5; the actor is the scheduler
rather than staff, which is why the audit row has no `actor_user_id`.

### D-016 — Table privileges are granted explicitly, never inherited

RLS and `GRANT` are two independent halves of one control: a policy without a
grant denies everything, a grant without a policy exposes everything. Supabase's
default privileges on `public` did **not** extend to tables created by our
migrations — every table came out with no `SELECT` for `anon`, `authenticated`
*or* `service_role`, which meant the entire Data API and every Edge Function
would have failed at runtime while every RLS test still passed.

The whole privilege surface is therefore stated explicitly in
`supabase/migrations/20260826090900_grant_table_privileges.sql`, mirroring the
policy matrix in `docs/05` §8 verb for verb. Three tables use column grants
